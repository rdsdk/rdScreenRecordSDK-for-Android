...
import com.rd.recsdk.utils.DateTimeUtils;
...
public class PlayActivity extends Activity {
	...
	private Runnable m_getPlayProgressRunnable = new Runnable() {
		...
		public void run() {
			...
			// Log.e("curr....", nPosition + "...." + duration);
			vvPlay.postDelayed(this, 100);
			videoProgressBar
	...
	private void initVedioView() {
		...
		vvPlay.setOnCompletionListener(new IMediaPlayer.OnCompletionListener() {
			...
			public void onCompletion(IMediaPlayer mp) {
				Log.e("onCompletion....", " ....." + mp.getDuration());
				if (vvPlay != null) {
		...
		vvPlay.setOnErrorListener(new IMediaPlayer.OnErrorListener() {
			...
			public boolean onError(IMediaPlayer mp, int what, int extra) {
				...
				if (what == -10000 && !(playTag == LiveType.mp4)) {
					onExitLiving("主播已退出房间");
				} else {
					onExitLiving("播放失败!");
				}
